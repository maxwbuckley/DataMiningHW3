...
On further exploration of the data I noted that beginning at approximatly the start November 2014 several sensors daily recording values fell to 0s. The columns for "N11Montrose", "OUTAwayFromCityCentre",  
"INTowardsCityCentre", "RockRoadPark", "RockRoadParkIN", "RockRoadParkOUT" all show this same behaviour. This is a serious data quality issue and will effect any forecasting or aggregation done on this data. 
...
You can see in the print outs above how my models did against the withheld training set. The mean absolute error for the Holt-Winters model was superior then the baseline average model in most instances. In the instances in which Holt-Winters was outperformed by the mean model there was usually a root cause. Almost all those sites with the data issues I mentioned above ("N11Montrose", "OUTAwayFromCityCentre",  
...
Taking for example the case of the "INTowardsCityCentre" column which I mentioned above has a problem with a string of 0 values in the end of 2014/ start of 2015
...
# the data is turning down. When it is in fact likely to be a recording anomaly.
train
